...
- 兼容性调整：将最低 go 版本由 1.18 调整为 1.19，因为本库使用了在 1.19 引入的 atomic.Pointer
- 转 string 规则优化：如果 `*T` 实现了 Stringer 接口，`T` 转 string 时也会调用 String 方法

### Fixed

- 修复bug：修复了某些情况下指针可能会指向引用栈上指针导致 panic 的问题
- 修复bug：修复了 mapassign_fast32 等函数的调用条件与 runtime 不一致的问题，改为与标准库一致且仅调用 faststr 版本
