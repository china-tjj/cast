...
func (h nativeMapHelper[K, V]) Load(m map[any]any, key unsafe.Pointer) (unsafe.Pointer, bool) {
...
func (h nativeMapHelper[K, V]) Range(m map[any]any, f func(key, value unsafe.Pointer) bool) {
...
func (h *reflectMapHelper) Make(mapAddr unsafe.Pointer, n int) map[any]any {
	...
	m := ref[unsafe.Pointer, map[any]any](getValueAddr(mv))
	*(*map[any]any)(mapAddr) = m
	return m
}

func (h *reflectMapHelper) Load(m map[any]any, key unsafe.Pointer) (unsafe.Pointer, bool) {
...
func (h *reflectMapHelper) Range(m map[any]any, f func(key, value unsafe.Pointer) bool) {
	mv := reflect.NewAt(h.mapType, unsafe.Pointer(&m)).Elem()
	iter := mv.MapRange()
	for iter.Next() {
